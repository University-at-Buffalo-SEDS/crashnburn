...
// ==============================
// Note: Besides the macros, everything in this file is designed to match the
// Rust API for easy mapping into C code. It contains no state or complex logic
// nor any live data. Any changes to the Rust API must be reflected here as
// well.
// ==============================
...
typedef enum SedsDataType {
  ...
  SEDS_DT_GPS_DATA = 1,
  SEDS_DT_GYROSCOPE_DATA = 2,
  SEDS_DT_ACCELEROMETER_DATA = 3,
  SEDS_DT_BATTERY_STATUS = 4,
  SEDS_DT_SYSTEM_STATUS = 5,
  SEDS_DT_BAROMETER_DATA = 6,
} SedsDataType;
...
typedef enum {
  ...
  SEDS_BAD_ARG =
      -2, /**< One or more arguments are invalid (NULL/size mismatch/etc). */
  SEDS_INVALID_TYPE = -3, /**< Unsupported/unknown type or endpoint value. */
  SEDS_SIZE_MISMATCH =
      -4, /**< Element/byte length does not match schema requirement. */
  SEDS_DESERIALIZE = -5, /**< Failed to deserialize a packet from bytes. */
  SEDS_HANDLER_ERROR =
      -6, /**< A user-provided endpoint handler returned an error. */
} SedsResult;

/**
 * @brief Monotonic clock callback: must return milliseconds since an arbitrary
 * epoch.
 *
 ...
 *
 * The view borrows data owned by the router. Fields must not be retained past
 * the callback that provided the view. Strings are not NUL-terminated unless
 * specified.
 */
 ...
 */
typedef SedsResult (*SedsTransmitFn)(const uint8_t *bytes, size_t len,
                                     void *user);
 ...
 */
typedef SedsResult (*SedsEndpointHandlerFn)(const SedsPacketView *pkt,
                                            void *user);
...
/**
 * @brief Get required buffer length for @ref seds_pkt_header_string (includes
 * NUL).
 *
 ...
 *
 * @note Use this to size your buffer prior to calling @ref
 * seds_pkt_header_string.
 */
 ...
 *
 * If @p buf is NULL or @p buf_len == 0, no data is written and the function
 * returns the required buffer length **including** the terminating NUL. If @p
 * buf is non-NULL, up to
 * (@p buf_len - 1) characters are written and the output is always
 ...
 * @return Required length including NUL (>=1) on success, or a negative @ref
 * SedsResult.
 *
 * @retval SEDS_BAD_ARG If @p pkt is NULL, or @p buf is non-NULL but @p buf_len
 * is 0.
 */
int32_t seds_pkt_header_string(const SedsPacketView *pkt, char *buf,
                               size_t buf_len);
 ...
 * SedsResult.
 */
int32_t seds_pkt_to_string(const SedsPacketView *pkt, char *buf,
                           size_t buf_len);
...
/**
 * @brief Create a router with an optional transmit callback and an array of
 * local handlers.
 *
 * @param tx         Optional transmit function (NULL if no remote transmit).
 * @param tx_user    Opaque user pointer for @p tx.
 * @param now_ms_cb  Callback to get current time in milliseconds (must not be
 * NULL).
 * @param handlers   Array of handler descriptors (may be NULL if @p
 * n_handlers==0).
 ...
 * @warning This function does not copy @p handlers; it copies only the
 * descriptors’ values.
 */
SedsRouter *seds_router_new(SedsTransmitFn tx, void *tx_user,
...
/**
 * @brief Log a typed slice; the router encodes elements to little-endian bytes
 * internally.
 *
 * The total number of bytes (count * elem_size) **must** match the schema size
 * for @p ty. Supported element sizes are 1, 2, 4, and 8 bytes.
 *
 * @param r          Router handle (must not be NULL).
 * @param ty         Telemetry data type.
 * @param data       Pointer to the first element of the typed slice (must not
 * be NULL if count>0).
 ...
 * @retval SEDS_SIZE_MISMATCH If the total bytes do not match the schema for @p
 * ty.
 * @note This call logs immediately to all relevant local handlers and/or queues
 * a copy for TX, depending on router configuration.
 */
SedsResult seds_router_log_typed(SedsRouter *r, SedsDataType ty,
                                 const void *data, size_t count,
                                 size_t elem_size, SedsElemKind elem_kind);

/**
 * @brief Queue a typed slice to the TX queue (no immediate transmit).
 *
 * Same constraints as @ref seds_router_log_typed. Use @ref
 * seds_router_process_send_queue to flush the queued packets through the
 * transmit callback.
 *
 * @param r          Router handle (must not be NULL).
 * @param ty         Telemetry data type.
 ...
 * @return SEDS_OK on success, or a negative @ref SedsResult.
 */
SedsResult seds_router_log_queue_typed(SedsRouter *r, SedsDataType ty,
                                       const void *data, size_t count,
                                       size_t elem_size,
 ...
 */
SedsResult seds_router_log_bytes(SedsRouter *r, SedsDataType ty,
 ...
 * @return SEDS_OK on success, or a negative @ref SedsResult.
 */
SedsResult seds_router_log_f32(SedsRouter *r, SedsDataType ty,
                               const float *vals, size_t n_vals);
 ...
 */
SedsResult seds_router_receive_serialized(SedsRouter *r, const uint8_t *bytes,
                                          size_t len);
 ...
 * @return SEDS_OK on success, or a negative @ref SedsResult.
 *
 * @note The router copies the data it needs; ownership of @p view remains with
 * the caller.
 */
SedsResult seds_router_receive(SedsRouter *r, SedsPacketView *view);

/**
 * @brief Process all packets in the TX queue: serialize and send them out.
 *
 * If a transmit callback was provided at router creation, it is invoked for
 * each packet. If the callback returns an error, processing stops and the error
 * is returned.
 *
 * If no transmit callback was provided, this function is a no-op that returns
 * SEDS_OK.
 *
 * @param r Router handle.
 ...
 * @return SEDS_OK on success, or a negative @ref SedsResult.
 *
 * @note If a handler returns an error, processing stops and that error is
 * returned.
 */
SedsResult seds_router_process_received_queue(SedsRouter *r);

/**
 * @brief Deserialize and queue a serialized packet into the RX queue (do not
 * process).
 *
 * @param r     Router handle.
 * @param bytes Serialized bytes.
 * @param len   Number of bytes.
 * @return SEDS_OK on success, or a negative @ref SedsResult (e.g.,
 * SEDS_DESERIALIZE).
 *
 * @note Use @ref seds_router_process_received_queue to deliver queued RX
 * packets.
 */
 ...
 * @return SEDS_OK if processing completed within the timeout or there was
 * nothing to send; otherwise a negative @ref SedsResult (e.g., callback error).
 *
 * @note Time measurement uses the router’s @ref SedsNowMsFn.
 * @note Timeout is not guaranteed to exit at the timeout specified, function
 * will return as soon as the current action has finished after the timeout has
 * elapsed. An example is if the current action takes 50ms but the timeout is
 * 5ms, one item will be handled in 50ms and the function will return leaving
 * any other items to be handled later.
 */
SedsResult seds_router_process_tx_queue_with_timeout(SedsRouter *r,
                                                     uint32_t timeout_ms);
 ...
 * any other items to be handled later.
 */
SedsResult seds_router_process_rx_queue_with_timeout(SedsRouter *r,
                                                     uint32_t timeout_ms);

/**
 * @brief Process both RX and TX queues until empty or until @p timeout_ms
 * elapses.
 *
 * @param r           Router handle.
 * @param timeout_ms  Maximum processing time in milliseconds.
 ...
 * any other items to be handled later.
 */
SedsResult seds_router_process_all_queues_with_timeout(SedsRouter *r,
                                                       uint32_t timeout_ms);
 ...
 *
 * @retval SEDS_SIZE_MISMATCH If the payload size is not a multiple of
 * sizeof(float) or does not match the schema for the packet’s type.
 */
 ...
 *
 * @warning The returned pointer is only valid for the lifetime of @p pkt (i.e.,
 * during the callback that provided the view). Do not retain it after the
 * callback returns.
 */
const void *seds_pkt_bytes_ptr(const SedsPacketView *pkt, size_t *out_len);

/**
 * @brief Get a direct pointer to the payload, validated as an array of
 * fixed-size elements.
 *
 * Validates that @p elem_size is one of {1,2,4,8} and that payload_len is an
 * exact multiple of @p elem_size. If validation passes, returns the same
 * underlying pointer as
 * @ref seds_pkt_bytes_ptr and (optionally) the element count.
 *
 * @param pkt        Packet view (must not be NULL).
 * @param elem_size  Element size in bytes (must be 1, 2, 4, or 8).
 * @param out_count  Optional; if non-NULL, receives payload_len / elem_size on
 * success (0 on fail).
 * @return Pointer to the payload bytes, or NULL if arguments are invalid or
 * sizes don’t divide evenly.
 *
 * @note No endianness conversion is performed. If interpreting multi-byte
 * scalars on a big-endian host, convert from little-endian after casting.
 * @warning The returned pointer is only valid for the lifetime of @p pkt (see
 * warning above).
 */
const void *seds_pkt_data_ptr(const SedsPacketView *pkt, size_t elem_size,
                              size_t *out_count);
 ...
 * @param out       Destination buffer for elements (must hold @p count
 * elements).
 ...
 * @return SEDS_OK on success; a negative @ref SedsResult on error.
 *
 * @retval SEDS_SIZE_MISMATCH If (count * elem_size) does not match the payload
 * length or the schema for the packet’s @p ty.
 * @note Elements are read in little-endian order from the payload.
 */
SedsResult seds_pkt_get_typed(const SedsPacketView *pkt, void *out,
...
template <> struct elem_traits<uint8_t> {
  static constexpr SedsElemKind kind = SEDS_EK_UNSIGNED;
  static constexpr size_t size = 1;
};

template <> struct elem_traits<uint16_t> {
  static constexpr SedsElemKind kind = SEDS_EK_UNSIGNED;
  static constexpr size_t size = 2;
};

template <> struct elem_traits<uint32_t> {
  static constexpr SedsElemKind kind = SEDS_EK_UNSIGNED;
  static constexpr size_t size = 4;
};

template <> struct elem_traits<uint64_t> {
  static constexpr SedsElemKind kind = SEDS_EK_UNSIGNED;
  static constexpr size_t size = 8;
};

template <> struct elem_traits<int8_t> {
  static constexpr SedsElemKind kind = SEDS_EK_SIGNED;
  static constexpr size_t size = 1;
};

template <> struct elem_traits<int16_t> {
  static constexpr SedsElemKind kind = SEDS_EK_SIGNED;
  static constexpr size_t size = 2;
};
...
template <> struct elem_traits<int64_t> {
  static constexpr SedsElemKind kind = SEDS_EK_SIGNED;
  static constexpr size_t size = 8;
};

template <> struct elem_traits<float> {
...
/**
 * @brief C++ convenience wrapper that deduces element kind/size from @p T and
 * logs immediately.
 */
template <typename T>
static SedsResult seds_router(SedsRouter *router, SedsDataType datatype,
                              const T *data, size_t count) {
  return seds_router_log_typed(
      router, datatype, static_cast<const void *>(data), count,
      seds_detail::elem_traits<T>::size, seds_detail::elem_traits<T>::kind);
}
 ...
 */
template <typename T>
static SedsResult seds_router_queue(SedsRouter *router, SedsDataType datatype,
                                    const T *data, size_t count) {
  return seds_router_log_queue_typed(
      router, datatype, static_cast<const void *>(data), count,
      seds_detail::elem_traits<T>::size, seds_detail::elem_traits<T>::kind);
}
 ...
 */
template <typename T>
static SedsResult seds_pkt_get(const SedsPacketView *pkt, T *out,
                               size_t count) {
  return seds_pkt_get_typed(pkt, static_cast<void *>(out), count,
 ...
 */
#define SEDS__KIND_SIZE(x, KIND_OUT, SIZE_OUT)                                 \
  do {                                                                         \
 ...
 */
#define SEDS__PKT_KIND_SIZE(x, KIND_OUT, SIZE_OUT)                             \
  SEDS__KIND_SIZE(x, KIND_OUT, SIZE_OUT)
