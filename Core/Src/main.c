...
*/
enum { STACK_DEFAULT = 512, STACK_SENSOR = 1024 * 2, STACK_DISPATCH = 512 * 2 };
...
uint8_t router_lock_flag = 0;
void router_lock(){
  while(router_lock_flag){
...
static void MX_GPIO_Init(void)
{
  ...
  /* CS pins high */
  HAL_GPIO_WritePin(GPIOB, accel_CS_Pin | gyro_CS_Pin | baro_CS_Pin, GPIO_PIN_SET);
  /* LED low */
...
static void SensorTask(void *arg)
{
  ...
  for (;;) {
    ...
    if (r != SEDS_OK) { print_telemetry_error(r); }
...
void blink(uint32_t delay_ms)
{
  ...
  while (1) {
    HAL_GPIO_TogglePin(led_GPIO_Port, led_Pin);
    for (volatile uint32_t i = 0; i < delay_ms * 1000; ++i) { __NOP(); }
  }
}
