...
HAL_StatusTypeDef baro_read_reg(SPI_HandleTypeDef *hspi, uint8_t reg,
  ...
  if (!out || len == 0 || len > 255) return HAL_ERROR;

  uint8_t head[2] = { (uint8_t)(reg | BMP390_SPI_READ_BIT), 0x00 };

  BARO_CS_LOW();
  HAL_StatusTypeDef st = HAL_SPI_Transmit(hspi, head, sizeof(head), BMP390_SPI_TIMEOUT_MS);
  if (st != HAL_OK) { BARO_CS_HIGH(); return st; }
...
HAL_StatusTypeDef baro_write_reg(SPI_HandleTypeDef *hspi, uint8_t reg,
  ...
  BARO_CS_LOW();
  HAL_StatusTypeDef st = HAL_SPI_Transmit(hspi, tx, (uint16_t)(len + 1), BMP390_SPI_TIMEOUT_MS);
  BARO_CS_HIGH();
  return st;
}
...
static HAL_StatusTypeDef baro_wait_idle(SPI_HandleTypeDef *hspi) {
  for (int i = 0; i < 20; ++i) {
    uint8_t s = 0;
    if (baro_read_u8(hspi, STATUS, &s) != HAL_OK) return HAL_ERROR;
    HAL_Delay(2);
  }
  return HAL_OK;
}
...
static HAL_StatusTypeDef baro_try_enter_normal(SPI_HandleTypeDef *hspi) {
  ...
  if (baro_write_u8(hspi, PWR_CTRL, (uint8_t)BMP390_PWR_NORMAL_WITH_SENSORS) != HAL_OK)
    return HAL_ERROR;
  HAL_Delay(BMP390_MODE_SWITCH_DELAY_MS);

  if (baro_read_u8(hspi, PWR_CTRL, &rb) != HAL_OK) return HAL_ERROR;
...
static HAL_StatusTypeDef baro_wait_drdy(SPI_HandleTypeDef *hspi, uint32_t extra_ms) {
  uint8_t odr_sel = 0;
  if (baro_read_u8(hspi, ODR, &odr_sel) != HAL_OK) return HAL_ERROR;
  ...
  for (;;) {
    uint8_t s = 0;
    if (baro_read_u8(hspi, STATUS, &s) != HAL_OK) return HAL_ERROR;
    if ((s & BMP390_STATUS_BOTH_DRDY) == BMP390_STATUS_BOTH_DRDY) return HAL_OK;
...
static HAL_StatusTypeDef baro_check_drdy(SPI_HandleTypeDef *hspi) {
  uint8_t s = 0;
  if (baro_read_u8(hspi, STATUS, &s) != HAL_OK) return HAL_ERROR;
  return ((s & BMP390_STATUS_BOTH_DRDY) == BMP390_STATUS_BOTH_DRDY) ? HAL_OK : HAL_BUSY;
}
...
HAL_StatusTypeDef init_barometer(SPI_HandleTypeDef *hspi) {
  ...
  // Reset
  {
    ...
    if (st != HAL_OK) die("baro: failed to issue soft reset\r\n");
    HAL_Delay(BMP390_RESET_DELAY_MS);
  }

  // Check ID
  st = baro_read_u8(hspi, CHIP_ID, &v);
  if (st != HAL_OK) die("baro: failed to read CHIP_ID\r\n");
  if (v != BMP390_CHIP_ID_VALUE) die("baro: CHIP_ID=0x%02X (expected 0x60)\r\n", v);

  // Force 4-wire
  v = 0x00; (void)baro_write_reg(hspi, IF_CONF, &v, 1);
  HAL_Delay(BMP390_ENABLE_DELAY_MS);
  ...
  // Read calibration
  if (read_trim_pars(hspi) != HAL_OK) die("baro: read calibration failed\r\n");
  ...
  for (int i = 0; i < 5; ++i) {
    if (baro_wait_drdy(hspi, 5) == HAL_OK &&
      ...
      if (p >= BMP390_BARO_VALID_MIN_PA && p <= BMP390_BARO_VALID_MAX_PA) break;
    }
    HAL_Delay(10);
  }
...
static float compensate_pressure(uint32_t uncomp_press) {
  ...
  float partial_out2 = a * (calib_data.par_p1 +
                            calib_data.par_p2 * t +
  ...
  return partial_out1 + partial_out2 + partial_aa + partial_cu * calib_data.par_p11; // Pa
}

// ---- Public getters ----
HAL_StatusTypeDef get_temperature(SPI_HandleTypeDef *hspi, float *temperature_c) {
  uint8_t tbuf[3];
  if (baro_wait_drdy(hspi, 5) != HAL_OK) return HAL_TIMEOUT;
  ...
  if (temperature_c) *temperature_c = t_c;
  return HAL_OK;
}
...
HAL_StatusTypeDef get_temperature_pressure(SPI_HandleTypeDef *hspi,
  ...
  if (temperature_c) *temperature_c = t_c;
  if (pressure_pa)   *pressure_pa   = p_pa;
  return HAL_OK;
}
...
HAL_StatusTypeDef get_temperature_pressure_altitude(SPI_HandleTypeDef *hspi,
  ...
  if (altitude_m)    *altitude_m    = compute_relative_altitude(p);
  return HAL_OK;
}
...
float compute_relative_altitude(float pressure) {
  ...
  return 44330.0f * (1.0f - powf(pressure / ground_level_pressure, BMP390_HYPSOMETRIC_EXPONENT));
}

float get_relative_altitude(SPI_HandleTypeDef *hspi) {
  float p;
  if (get_pressure(hspi, &p) != HAL_OK) return -1.0f;
...
HAL_StatusTypeDef get_temperature_pressure_non_blocking(SPI_HandleTypeDef *hspi,
  ...
  if (baro_check_drdy(hspi) != HAL_OK) {
    if (temperature_c) *temperature_c = last_temp;
    if (pressure_pa)   *pressure_pa   = last_press;
    return HAL_OK;
  }
...
HAL_StatusTypeDef get_temperature_pressure_altitude_non_blocking(SPI_HandleTypeDef *hspi,
  ...
  if (baro_check_drdy(hspi) != HAL_OK) {
    if (temperature_c) *temperature_c = last_temp;
    if (pressure_pa)   *pressure_pa   = last_press;
    if (altitude_m)    *altitude_m    = compute_relative_altitude(last_press);
    return HAL_OK;
  }
  ...
  if (temperature_c) *temperature_c = t_c;
  if (pressure_pa)   *pressure_pa   = p_pa;
  if (altitude_m)    *altitude_m    = compute_relative_altitude(p_pa);
  return HAL_OK;
}
